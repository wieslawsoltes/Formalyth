...
export function installModelCommands(ctx){
 ...
 c('sketch.create','Create sketch','sketch','Sketch',async supplied=>{
   if(supplied?.loops)return w.addFeature('region',{loops:supplied.loops},[],'Closed sketch regions');
   if(ctx.getEditor())return;
   const editor=new RegionEditor(document.querySelector('#viewport'),{onFinish:async loops=>{await w.addFeature('region',{loops},[],'Closed sketch regions');ctx.setEditor(null);toast('Sketch saved. Select Extrude to create a solid.');},onCancel:()=>ctx.setEditor(null)});ctx.setEditor(editor);
 });
