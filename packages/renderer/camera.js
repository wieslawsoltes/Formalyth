import {perspective} from './projection.js';
import {clamp, m4, v3} from '../math/index.js';
/** Orthographic Z-up CAD camera. Scale is vertical world-space span. */
export class OrbitCamera {
  constructor() { this.target = [0, 0, 0]; this.scale = 130; this.yaw = -.8; this.pitch = .65; this.aspect = 1; this.projection = 'orthographic'; this.fov = 45; }
  get eye() { const d = this.projection==='perspective'?this.scale/(2*Math.tan(this.fov*Math.PI/360)):this.scale*4+100, c = Math.cos(this.pitch); return v3.add(this.target, [d*c*Math.cos(this.yaw), d*c*Math.sin(this.yaw), d*Math.sin(this.pitch)]); }
  get viewMatrix() { return m4.lookAt(this.eye, this.target); }
  matrix(zeroToOne = false) { if(this.projection==='perspective')return m4.multiply(perspective(this.fov*Math.PI/180,this.aspect,Math.max(.001,this.scale*1e-4),this.scale*20+1000,zeroToOne),this.viewMatrix); const h = this.scale/2; return m4.multiply(m4.ortho(-h*this.aspect, h*this.aspect, -h, h, .001, this.scale*20+1000, zeroToOne), this.viewMatrix); }
