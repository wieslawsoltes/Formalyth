...
const report={node:process.version,platform:process.platform,cpu:os.cpus()[0]?.model,runs,cache:{bearing:{computed:warm.stats.computed,reused:warm.stats.reused,transferredOutputs:warm.changes.length},many:{computed:delta.stats.computed,reused:delta.stats.reused,transferredOutputs:delta.changes.length}},note:'CPU execution samples, not interactive FPS or hardware-GPU benchmarks.'};
